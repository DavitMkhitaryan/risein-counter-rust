    ...
    match instruction {
        ...
        CounterInstructions::Decrement(args) => {
            if counter_account.counter >= args.value {
                ...
                return Err(ProgramError::InvalidInstructionData); // or some custom error
            }
        }
...
mod test {
    ...
    fn test_counter() {
        ...
        // let decrement_value_2 = 10u32;
        // decrement_instruction_data.extend_from_slice(&decrement_value_2.to_le_bytes());
        // process_instruction(&program_id, &accounts, &decrement_instruction_data).unwrap();
        // assert_eq!(
        //     CounterAccount::try_from_slice(&accounts[0].data.borrow())
        //         .unwrap()
